        ...
        if constexpr (ndim == 1){
            ...
            for(const Element &el : fespace.elements) {
                for(int isample = 0; isample <= 100; ++isample){
                    ...
                    Point phys_pt{};
                    el.transform(coord, pt, phys_pt);


                    // calculate the jacobian determinant
                    auto J = el.geo_el->Jacobian(coord, pt);
                    T detJ = NUMTOOL::TENSOR::FIXED_SIZE::determinant(J);
    ...
    T ic_error(
        ...
        for(const Trace& trace : fespace.get_interior_traces()) {
            ...
            for(int iqp = 0; iqp < trace.nQP(); ++iqp){
                ...
                auto Jfac = trace.face->Jacobian(fespace.meshptr->nodes, quadpt.abscisse);
                ...
                auto gradBiL = trace.evalPhysGradBasisQPL(iqp, fespace.meshptr->nodes, gradbL_data.data());
                auto gradBiR = trace.evalPhysGradBasisQPR(iqp, fespace.meshptr->nodes, gradbR_data.data());
                        ...
                        case BOUNDARY_CONDITIONS::DIRICHLET:
                            {
                                ...
                                trace.face->transform(quadpt.abscisse, fespace.meshptr->nodes, phys_pt);
